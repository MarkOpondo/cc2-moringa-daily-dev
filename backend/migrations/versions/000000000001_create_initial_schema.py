...
def upgrade():
    # Older local checkouts created these tables with db.create_all without
    # an Alembic version row. Treat that schema as the migration baseline so
    # pending column migrations can repair it instead of recreating tables.
    existing_tables = set(sa.inspect(op.get_bind()).get_table_names())
    if {"users", "content"}.issubset(existing_tables):
        return
